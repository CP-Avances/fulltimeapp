...
const BuscarTimbresByFecha = function (fec_inicio, fec_final) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield database_1.pool.query('SELECT fec_hora_timbre FROM timbres WHERE CAST(fec_hora_timbre AS VARCHAR) between $1 || \'%\' AND $2 || \'%\' ORDER BY fec_hora_timbre ASC', [fec_inicio, fec_final])
            .then(res => {
            return res.rows;
        });
    });
...
const BuscarHorariosActivos = function (fec_inicio, fec_final) {
    return __awaiter(this, void 0, void 0, function* () {
        let lista_horarios = yield database_1.pool.query('SELECT * FROM empl_horarios WHERE CAST(fec_inicio AS VARCHAR) between $1 || \'%\' AND $2 || \'%\' ORDER BY fec_inicio ASC', [fec_inicio, fec_final])
            .then(res => {
            return res.rows;
        });
...
const BuscarPermisosJustificados = function (codigo, fecha) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield database_1.pool.query('SELECT fec_inicio, descripcion FROM permisos WHERE codigo = $1 AND fec_inicio::TIMESTAMP::DATE <= $2 AND fec_final::TIMESTAMP::DATE >= $2 AND estado = 3 ', [codigo, fecha + ''])
            .then(result => {
...
function HorasExtrasSolicitadasGrafica(fec_desde, fec_hasta) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield database_1.pool.query('SELECT CAST(h.fec_inicio AS VARCHAR), CAST(h.fec_final AS VARCHAR), h.descripcion, h.num_hora, h.tiempo_autorizado, h.codigo, h.id_empl_cargo ' +
...
function PlanificacionHorasExtrasSolicitadasGrafica(fec_desde, fec_hasta) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield database_1.pool.query('SELECT CAST(h.fecha_desde AS VARCHAR), CAST(h.hora_inicio AS VARCHAR), h.fecha_hasta, h.hora_fin, h.descripcion, h.horas_totales, ph.tiempo_autorizado, ph.codigo, ph.id_empl_cargo ' +
...
const BuscarTimbresEntradasSinAcciones = function (fec_inicio, fec_final) {
    return __awaiter(this, void 0, void 0, function* () {
        const orden = 1;
        const horarioEntrada = yield database_1.pool.query('SELECT eh.codigo, dt.hora, dt.minu_espera, CAST(eh.fec_inicio AS VARCHAR), CAST(eh.fec_final AS VARCHAR), ' +
...
function EmpleadoHorasExtrasSolicitadasGrafica(codigo, fec_desde, fec_hasta) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield database_1.pool.query('SELECT h.fec_inicio, h.fec_final, h.descripcion, h.num_hora, h.tiempo_autorizado, h.codigo, h.id_empl_cargo ' +
            'FROM hora_extr_pedidos AS h WHERE h.fec_inicio between $1 and $2 AND h.estado = 3 ' + // estado = 3 significa q las horas extras fueron autorizadas
...
function EmpleadoPlanificacionHorasExtrasSolicitadasGrafica(codigo, fec_desde, fec_hasta) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield database_1.pool.query('SELECT h.fecha_desde, h.hora_inicio, h.fecha_hasta, h.hora_fin, h.descripcion, h.horas_totales, ph.tiempo_autorizado, ph.codigo, ph.id_empl_cargo ' +
            'FROM plan_hora_extra_empleado AS ph, plan_hora_extra AS h WHERE ph.id_plan_hora = h.id AND ph.estado = 3 ' + //estado = 3 para horas extras autorizadas
...
const Empleado_Vacaciones_ModelarDatos = function (codigo, fec_desde, fec_hasta) {
    return __awaiter(this, void 0, void 0, function* () {
        let vacaciones = yield database_1.pool.query('SELECT CAST(fec_inicio AS VARCHAR), CAST(fec_final AS VARCHAR) FROM vacaciones WHERE codigo = $1 AND fec_inicio between $2 and $3 AND estado = 3 ', [codigo, fec_desde, fec_hasta]).then(result => { return result.rows; });
...
const Empleado_Permisos_ModelarDatos = function (codigo, fec_desde, fec_hasta) {
    return __awaiter(this, void 0, void 0, function* () {
        let permisos = yield database_1.pool.query('SELECT CAST(fec_inicio AS VARCHAR), CAST(fec_final AS VARCHAR), hora_numero, dia FROM permisos WHERE codigo = $1 AND fec_inicio between $2 and $3 AND estado = 3 ', [codigo, fec_desde, fec_hasta]).then(result => { return result.rows; });
...
const Empleado_Atrasos_ModelarDatos_SinAcciones = function (codigo, fec_desde, fec_hasta) {
    return __awaiter(this, void 0, void 0, function* () {
        const orden = 1;
        const horarioEntrada = yield database_1.pool.query('SELECT dt.hora, dt.minu_espera, CAST(eh.fec_inicio AS VARCHAR), CAST(eh.fec_final AS VARCHAR), ' +
            'eh.lunes, eh.martes, eh.miercoles, eh.jueves, eh.viernes, eh.sabado, eh.domingo ' +
            'FROM empl_horarios AS eh, cg_horarios AS ch, deta_horarios AS dt ' +
            'WHERE dt.orden = $1 AND eh.fec_inicio BETWEEN $2 AND $3 AND eh.fec_final BETWEEN $2 AND $3 ' +
...
const BuscarTimbresSinAccionesReporte = function (fecha, codigo) {
    return __awaiter(this, void 0, void 0, function* () {
        return yield database_1.pool.query('SELECT CAST(fec_hora_timbre AS VARCHAR) FROM timbres WHERE CAST(fec_hora_timbre AS VARCHAR) like $1 || \'%\' AND id_empleado = $2 ORDER BY fec_hora_timbre ASC ', [fecha, codigo])
            .then(res => {
            return res.rows;
        });
    });
