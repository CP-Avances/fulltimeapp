...
@Component({
  ...
  templateUrl: './permiso-editar-solicitud.page.html',
  styleUrls: ['./permiso-editar-solicitud.page.scss'],
})
export class PermisoEditarSolicitudPage implements OnInit {
  ...
  private readonly TIPO_NOTI_PERMISO_EDICION = TipoNotificacion.EDITAR_PERMISO;
  ...
  ngOnInit() {
    ...
    if (!solicitudState?.id) {
      this.mostrarToast('No se recibió la solicitud para editar.', 'warning');
      this.regresar();
      return;
    }

    this.solicitud = solicitudState;
    this.idSolicitud = Number(solicitudState.id);

    this.idEmpleado = Number(
      solicitudState.id_empleado ||
  ...
  cargarDetalleSolicitud() {
    ...
    this.permisosService.obtenerSolicitudPermisoPorId(this.idSolicitud).subscribe({
      next: (detalle) => {
        if (detalle) {
          this.solicitud = {
            ...this.solicitud,
  ...
  cargarDatosEnFormulario() {
    ...
    this.idEmpleado = Number(
      this.solicitud.id_empleado ||
      this.idEmpleado ||
      localStorage.getItem('empleadoID') ||
      0
    );

    this.tipoPermisoSeleccionado = Number(
      this.solicitud.id_tipo_permiso ||
      this.solicitud.id_tipo_solicitud ||
      0
    );

    this.observacion = this.solicitud.descripcion || '';
    this.incluirFeriadosSeleccionado = !!this.solicitud.incluir_feriados;
    this.documentoActual = this.solicitud.documento || '';

    const minutosTotales = Number(this.solicitud.minutos_totales || 0);
    const tieneHoras = minutosTotales > 0 || !!this.solicitud.hora_inicio || !!this.solicitud.hora_fin;

    if (tieneHoras) {
      this.modoSolicitud = 'HORAS';
      this.permiteHoras = true;

      this.fechaHoras = this.obtenerFechaInput(this.solicitud.fecha_inicio);
      this.horaInicio = this.normalizarHora(this.solicitud.hora_inicio);
      this.horaFinal = this.normalizarHora(this.solicitud.hora_fin);
      ...
      this.fechaInicio = this.obtenerFechaInput(this.solicitud.fecha_inicio);
      this.fechaFinal = this.obtenerFechaInput(this.solicitud.fecha_final);
      ...
      this.conteoDiasSemana = {
        ...
        S: Number(this.solicitud.numero_dias_sabados || 0),
        D: Number(this.solicitud.numero_dias_domingos || 0),
      };

      this.diasTotales = Number(
    ...
    this.horasTotales = this.getHorasFormatoHHmmDesdeMinutos(minutosTotales);
  ...
  cargarTiposPermiso() {
    ...
    this.permisosService.listarTiposPermiso().subscribe({
      next: (data) => {
        ...
        this.tiposPermiso = lista.filter((p: any) => {
          const idSucursalTipo = Number(p.sucursal_id ?? p.id_sucursal);
          return Number(idSucursalTipo) === Number(this.idSucursal);
        });

        const tipoActual = Number(
          this.solicitud?.id_tipo_permiso ||
          this.solicitud?.id_tipo_solicitud ||
          0
        );

        const existeTipoActual = this.tiposPermiso.some(
          (t: any) => Number(t.id) === tipoActual
        );

        if (!existeTipoActual && tipoActual) {
          const encontrado = lista.find((t: any) => Number(t.id) === tipoActual);
          if (encontrado) {
            this.tiposPermiso.push(encontrado);
          }
        }
  ...
  guardarCambios() {
    ...
    if (this.requiereDocumento && !this.archivoSeleccionado && !this.documentoActual) {
    ...
    this.permisosService.actualizarSolicitudPermiso(this.idSolicitud, payload).subscribe({
      next: async (response) => {
        ...
        if (this.archivoSeleccionado) {
          this.subirDocumentoNuevoConComunicacion(solicitudActualizada, tipo);
          return;
        }

        await this.enviarComunicacionesEdicionPermiso(
  ...
  private subirDocumentoNuevoConComunicacion(solicitudActualizada: any, tipo: any) {
    if (!this.archivoSeleccionado) {
      return;
    }
  ...
  private async enviarComunicacionesEdicionPermiso(
    solicitud: any,
    tipoPermiso: any,
    cambioDocumento: string | null = null
  ): Promise<void> {
    try {
      ...
      if (!idEmpleadoSolicitante || !idTipoPermiso || !idDepartamento) {
        console.warn('No se envía notificación de edición: faltan datos base.', {
      ...
      if (!flujoPermiso) {
        console.warn('No se encontró flujo para notificación de edición.', {
          idDepartamento,
          idTipoPermiso,
          flujos
        });
      ...
      const tipoPermiteCorreoEdicion = this.valorBooleanoPermiso(
        tipoPermiso?.correo_editar ?? tipoPermiso?.correo_edicion,
        true
      );
      ...
      const idsCorreo = empleadosReceptores
        .filter((e: any) => {
          ...
          return tipoPermiteCorreoEdicion && recibeCorreo && !!e?.correo;
        })
      ...
      const mensaje = this.armarMensajeEdicionPermiso(
        solicitud,
        tipoPermiso,
        empleadoSolicitante,
        cambioDocumento
      ...
      const idEnvia = Number(localStorage.getItem('empleadoID') || this.idEmpleado || 0);
        ...
        try {
          ...
          const payloadCorreo = {
            ...
            items: [
              {
                correo: correoUnico,
                asunto: 'Solicitud de permiso actualizada',
                mensaje,
          ...
          console.error('ERROR AL ENVIAR CORREO DE EDICION DE PERMISO', error);
        }
      }
      ...
      if (idsNotificacion.length > 0) {
        try {
          ...
          const payloadNotificacion = {
            ...
            mensaje,
            tipo: TipoNotificacion.EDITAR_PERMISO,
            id_permiso: idPermiso || null
          };
          ...
          console.error('ERROR AL ENVIAR NOTIFICACION DE EDICION DE PERMISO', error);
        }
      }

    } catch (error) {
      console.error('ERROR GENERAL AL ENVIAR COMUNICACIONES DE EDICION DE PERMISO', error);
    }
  }

  private armarMensajeEdicionPermiso(
    solicitud: any,
    tipoPermiso: any,
    ...
    const payloadMensaje = {
      accion: 'EDITADO',
      mensaje_principal: 'Se ha actualizado la siguiente solicitud de permiso:',
      notificacion: 'Se ha actualizado la siguiente solicitud de permiso:',
      data: {
        empleado: nombreEmp,
        ...
        realizado_por: localStorage.getItem('fullname') || nombreEmp,
  ...
  puedeGuardar(): boolean {
    return this.verificacionRealizada && this.estadoVerificacion === 'ok' && !this.guardando;
  }
  ...
  getHorasFormatoHHmmDesdeMinutos(minutos: number): string {
  ...
  obtenerFechaInput(fecha: any): string {
    if (!fecha) return '';
  ...
  normalizarHora(hora: any): string {
    if (!hora) return '';
  ...
  regresarBusquedaPermisos() {
    localStorage.setItem('resetBusquedaPermisos', 'true');

    this.router.navigateByUrl(
      '/reloj/solicitudes/permiso-solicitud/permiso-criterio-busqueda'
    );
  }
